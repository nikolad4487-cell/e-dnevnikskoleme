...
const modalStartIndex = content.indexOf('function GradingElementsModal');

const newModalCode = `function GradingElementsModal({ isOpen, onClose, subject, classId, schoolId, teacherId, onRefresh }: { isOpen: boolean, onClose: () => void, subject: Subject, classId: string, schoolId: string, teacherId: string, onRefresh: () => void }) {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      fetchElements();
      onRefresh();
      toast.success('Element uspješno ażuriran.');
    } catch (err) {
      toast.error('Greska pri ażuriranju.');
    }
  };
        ...
        <div className="p-6 space-y-6">
                ...
                {editingElementId === e.id ? (
                  <div className="flex items-center gap-2">
                    <input 
                      ...
                      className="flex-1 px-3 py-1.5 border-2 border-[#005c8d] bg-white text-xs font-bold text-slate-900 outline-none"
                      autoFocus
                    />
                    <button onClick={handleEdit} className="p-1.5 bg-[#005c8d] text-white hover:bg-[#004a70]"><Check size={14}/></button>
                    <button onClick={() => setEditingElementId(null)} className="p-1.5 bg-slate-200 text-slate-600 hover:bg-slate-300"><X size={14}/></button>
                  </div>
                ) : (
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-black text-slate-800 uppercase tracking-tight">{e.name}</span>
                    <div className="flex gap-1.5 items-center">
                       <button onClick={() => { setEditingElementId(e.id); setEditingElementName(e.name); }} className="text-[9px] font-black uppercase text-[#005c8d] hover:underline">Uredi</button>
          ...
          <div className="flex gap-2 pt-4 border-t border-slate-100">
            <input 
              ...
              className="flex-1 px-3 py-2 border-2 border-slate-200 bg-white text-xs font-bold text-slate-900 outline-none focus:border-[#005c8d]"
              onKeyDown={e => { if (e.key === 'Enter') handleAdd(); }}
            />
            <button onClick={handleAdd} className="bg-[#005c8d] text-white px-4 py-2 text-[10px] font-black uppercase hover:bg-[#004a70] transition-colors leading-none tracking-widest whitespace-nowrap">Dodaj element</button>
          </div>
        </div>
      </div>
    </div>
...
console.log('Fixed element modal!');
