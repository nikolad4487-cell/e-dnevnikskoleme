  ...
  useEffect(() => {
    if (selectedSchoolId && selectedYearId) {
      const fetchClasses = async () => {
        ...
        if (data) {
          setClasses(data);
          // If selected class is not in the new set, reset it
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-2">
           <div className="relative">
             ...
             {isSchoolMenuOpen && (
               <div className="absolute top-full mt-1 w-48 bg-white text-gray-800 border rounded shadow-xl z-50 py-1">
                 {schools.map(s => <button key={s.id} onClick={() => { setSelectedSchoolId(s.id); setIsSchoolMenuOpen(false); setSelectedYearId(null); setSelectedClassId(null); }} className="w-full text-left px-4 py-2 hover:bg-slate-100 text-xs">{s.name}</button>)}
               </div>
             )}
           </div>
           ...
           <div className="relative">
             ...
             {isYearMenuOpen && (
               <div className="absolute top-full mt-1 w-48 bg-white text-gray-800 border rounded shadow-xl z-50 py-1">
                 {schoolYears.map(y => <button key={y.id} onClick={() => { setSelectedYearId(y.id); setIsYearMenuOpen(false); setSelectedClassId(null); }} className="w-full text-left px-4 py-2 hover:bg-slate-100 text-xs">{y.name}</button>)}
               </div>
             )}
           </div>
           ...
           <div className="relative">
             ...
             {isClassMenuOpen && (
               <div className="absolute top-full mt-1 w-48 bg-white text-gray-800 border rounded shadow-xl z-50 py-1">
                 {classes.map(c => <button key={c.id} onClick={() => { setSelectedClassId(c.id); setIsClassMenuOpen(false); }} className="w-full text-left px-4 py-2 hover:bg-slate-100 text-xs">{c.name}</button>)}
               </div>
             )}
           </div>
        </div>
